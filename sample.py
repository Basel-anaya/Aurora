...
def apply_incremental_optimization(deformed_pcd, kinematic_column, max_iterations=DEFAULT_MAX_ITERATIONS, error_threshold=DEFAULT_ERROR_THRESHOLD):
    ...
    for iteration in range(max_iterations):
        ...
        print(f"  Average distance: {avg_distance:.6f}, Max distance: {max_distance:.6f}")
        
        # Early termination check
        if max_distance < error_threshold:
            print(f"  Reached error threshold ({error_threshold}). Terminating early.")
            break
        ...
        print(f"  Distribution of closest points by segment: {seg_distribution}")
        ...
        print(f"  Error after focusing on closest points: {current_error:.6f}")
        ...
        print(f"  Main bending axis: {main_axis} (0=X, 1=Y, 2=Z) with direction: {main_axis_direction}")
        ...
        for i in range(num_segments):
            for j in range(3):
                ...
                if abs(new_angle) > max_angle:
                    ...
                    if iteration == 0:  # Only log these warnings in first iteration to avoid spam
                        print(f"  Limiting segment {i}, axis {j} to {max_angle:.4f} radians")
        ...
        if iteration > 10 and current_error < error_threshold * 2:
            ...
            if len(full_correspondences) > 0:
                ...
                print(f"  Full evaluation max error: {full_max_error:.6f}")
                
                if full_max_error < error_threshold:
                    print("  Early convergence achieved. Terminating.")
                    break
...
def run_incremental_optimization_experiment(cross_section='circular', size=0.1):
    ...
    original_column = ArticulatedColumn(num_segments=10, segment_length=0.25, size=size, cross_section=cross_section)
    
    # Create ground truth deformed column with a clear bend
    deformed_column = copy.deepcopy(original_column)

    deformed_column.bend_at_segment(9, 'y', np.pi/24)
    deformed_column.bend_at_segment(8, 'y', np.pi/20)
    deformed_column.bend_at_segment(7, 'y', np.pi/20)
    deformed_column.bend_at_segment(6, 'y', np.pi/24)
    
    # Create point cloud from the deformed column
    deformed_pcd = column_to_point_cloud(deformed_column, total_points=10000)
    ...
    plt.figure(figsize=(12, 6))
    plt.plot(rmse_values, 'b-', linewidth=2)  # Blue line
    plt.yscale('log')
    plt.ylim(bottom=None, top=1e-1)  # Keep the bottom limit automatic, set top to 0.1
    plt.xlabel('Iteration', fontsize=20)
    plt.ylabel('RMSE', fontsize=20)
    plt.title('Root Mean Square Error (RMSE) per Iteration', fontsize=24, pad=20)
    y_ticks = [5e-3, 1e-2, 2e-2, 5e-2, 1e-1, 5e-1]  # Values up to 1e-1
    plt.yticks(y_ticks, ['{:.0e}'.format(y) for y in y_ticks], fontsize=10)
    plt.xticks(fontsize=10)
    plt.grid(True, linestyle='--', alpha=0.7)
    ax = plt.gca()
    ax.minorticks_on()
    # Add minor grid (optional)
    ...
    plot_node_displacements(original_column, result_column)
    plot_result_vs_groundtruth_error(deformed_column, result_column)
    ...
    display.FitAll()
    start_display() 
    return original_column, deformed_column, result_column, errors, rmse_values, frames, node_indices, displacements


if __name__ == "__main__":
    # Run with the new incremental optimization approach
    print("Running experiment with incremental optimization and circular cross-section...")
    original_column, deformed_column, result_column, errors, rmse_values, frames, node_indices, displacements = run_incremental_optimization_experiment(cross_section='h', size=(0.276, 0.261, 0.0153, 0.0253)  # (flange_width, web_height, web_thickness, flange_thickness)
)

#print("\nRunning experiment with incremental optimization and rectangular cross-section...")
#run_incremental_optimization_experiment(cross_section='rectangular', size=(0.2, 0.4))
